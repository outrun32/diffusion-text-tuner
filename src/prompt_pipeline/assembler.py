...
_TEMPLATES_RU = [
    ...
    (
        "{scene}. Надпись '{target_text}', цвет: {color_raw}, шрифт: "
        "{font_raw}{effect}. Высокое качество"
    ),
...
_TEMPLATES_EN = [
    ...
    (
        "{scene} with clearly readable text '{target_text}', {font_en} font, "
        "{color_en} color{effect_en}"
    ),
    (
        "{scene}. A {size_en_lower} {placement_en} reading '{target_text}', "
        "{color_en} {font_en}{effect_en}, detailed"
    ),
]
...
class Assembler:
    ...
    def _assemble_ru(self, target_text, scene, style, content_type) -> str:
        ...
        placement = self.rng.choice(_PLACEMENT_RU.get(content_type, ["текст"]))
    ...
    def _assemble_en(self, target_text, scene, style, content_type) -> str:
        ...
        placement = self.rng.choice(_PLACEMENT_EN.get(content_type, ["text"]))
