...
def parse_args():
    ...
    p.add_argument("--output-dir", type=str, default="outputs/baseline",
    ...
    p.add_argument("--num-samples", type=int, default=500,
                    help="Number of images to generate (0 = all)")
    p.add_argument("--batch-size", type=int, default=1,
                    help="Images per batch (>1 needs more VRAM)")
...
def main():
    ...
    try:
        for i in tqdm(range(start, total, bs), desc="Generating", unit="batch"):
            ...
            target_texts = [r.get("target_text", "") for r in batch]
            ...
            for j, (image, record) in enumerate(zip(results.images, batch)):
                idx = i + j
                fname = f"{idx:06d}.png"
                fpath = img_dir / fname
                image.save(fpath)
