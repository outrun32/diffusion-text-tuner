"""Historical CUDA profiler for one ReFL training step.

Run through ``python -m scripts.profile_step`` so importing the supported
wrapper never loads models or starts profiling.
"""
...
lora_config = PeftLoraConfig(
    r=64, lora_alpha=64, target_modules=["to_k", "to_q", "to_v", "to_out.0"], lora_dropout=0.0
)
...
nograd_pct = nograd_time / total_denoise * 100
grad_pct = grad_time / total_denoise * 100
cfg_pct = cfg_time / total_denoise * 100
print(f"No-grad forward   ({step_with_grad} steps):  {nograd_time:.2f}s  ({nograd_pct:.1f}%)")
print(f"Grad forward      (1 step):           {grad_time:.2f}s  ({grad_pct:.1f}%)")
print(f"CFG neg forward   ({step_with_grad + 1} steps): {cfg_time:.2f}s  ({cfg_pct:.1f}%)")
