...
- [ ] **ENV-01**: User can install the project from a committed dependency manifest for Python 3.11.
- [ ] **ENV-02**: User can choose optional dependency groups for GPU training, OCR/reward scoring, synthesis, vLLM/MLX backends, tests, linting, plotting, and analysis.
- [ ] **ENV-03**: User can run a standard CPU-safe test command without downloading large models or requiring CUDA.
...
- [ ] **ENV-05**: User can format and lint the repository with standard documented commands.
...
- [ ] **TEST-06**: User can distinguish default CPU tests from marked slow, GPU, model, OCR, integration, and manual diagnostic checks.
